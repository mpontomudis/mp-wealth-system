    ...
    <motion.div ref={ref} className={cn('relative mx-auto h-full w-full max-w-4xl', className)}>
      <div className="absolute left-0 top-3 hidden md:block">
        <motion.div
          ...
          className="ml-[27px] flex h-4 w-4 items-center justify-center rounded-full border border-mp-border bg-white"
        >
          <motion.div
            transition={{ duration: 0.2, delay: 0.5 }}
            animate={{ backgroundColor: scrollYProgress.get() > 0 ? 'white' : '#3b82f6', borderColor: scrollYProgress.get() > 0 ? 'white' : '#3b82f6' }}
            className="h-2 w-2 rounded-full border border-mp-border bg-white"
          />
        </motion.div>
        <svg
